...
- validated commit：`ffb79c451762e71baea43f4422283d2e25ef4c6c`
- 操作系统：Windows
- Node.js：`v22.17.0`
- pnpm：`10.29.3`
...
Set-Location "E:\澳门开发-worktrees\recover-current"; pnpm install --frozen-lockfile
...
Set-Location "E:\澳门开发-worktrees\recover-current"; pnpm --filter @mct/server test
Set-Location "E:\澳门开发-worktrees\recover-current"; pnpm --filter @mct/table-3d test
Set-Location "E:\澳门开发-worktrees\recover-current"; pnpm --filter @mct/room-client test
Set-Location "E:\澳门开发-worktrees\recover-current"; pnpm --filter @mct/room-protocol test
```
...
| `pnpm test` | 35 个测试文件、536 个测试通过 |
| `pnpm typecheck` | 7/7 workspace 项目成功 |
| `@mct/server` | 11 个测试文件、144 个测试通过 |
| `@mct/table-3d` | 14 个测试文件、177 个测试通过 |
...
```powershell
Set-Location "E:\澳门开发-worktrees\recover-current"; $env:DEMO_ROOM_CREDENTIAL = "<runtime-generated-temporary-value>"; $env:PORT = "<ephemeral-local-port>"; node apps/server/dist/index.js
```
...
- table-3d 生产 bundle 为 751.34 kB（gzip 196.47 kB），Vite 报告 chunk 大于 500 kB；这是已有性能 warning，不是构建失败。
